...
//    PrintData("sourceMsg", sourceMsg, strlen(sourceMsg));
    ...
    if(symbol=='E'){
        ...
        scanf("%s",&sourceMsg);
        if(strlen(sourceMsg)>128){
            printf("Sorry, but long-message encryption is not allowed in AES free edition. Please use Long/XL edition.\n");
            return;
        }
        PrintData("sourceMsg", sourceMsg, strlen(sourceMsg));
    ...
    if(symbol=='D'){
        ...
        scanf("%d",&Klen);
        if(Klen>512){
            printf("Sorry, but long-message decryption is not allowed in AES free edition. Please use Long/XL edition.\n");
            return;
        }
...
int main(void)
{
    printf("Welcome to AES-128 v1.1 Legacy.\nAES Series by @euyoBnaW\n");
    AES_KEY_GET();
    while(1){
        my_aes_unit();
